...
module.exports = (program) => {
  ...
  app.post('/profiles/run/:profile', (req, res) => {
    ...
    prefontaine(prog)
      ...
      .then(function (pre) {
        // quick hack
        let reportUrl = `http://localhost:3000/report/${pre.config.get('output:reports').split('test/report/')[1]}/summary.json`
        res.json({
          launched: true,
          summary: reportUrl
        });
  ...
  prefontaine(program)
    .then(function (pre) {
      app.use('/report', express.static(pre.config.get('output:reports'), {index: false}));
