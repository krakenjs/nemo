...
const logkill = debug('nemo:mocha:destroy');
function MochaRunner(runnerConfig) {
  ...
  function createNemo() {
    ...
    } else if (typeof instanceConfig.profile.conf.driver === 'function') {
      log('profile level driver override as function');
      driverConf = instanceConfig.profile.conf.driver;
    }
  ...
  async function destroyNemo() {
    logkill('destroyNemo: called');
    if (this.nemo && this.nemo.driver && this.nemo.driver.quit) {
      logkill('Quitting nemo driver instance');
      try {
        logkill('try to kill driver')
        await this.nemo.driver.quit();
      } catch (err) {
        logkill('failed to kill driver');
        logkill(err);
        return;
      }
      logkill('succeeded to kill driver');
    } else {
      logkill('Nemo instance does not have a driver to quit');
    }
    return;
  }
