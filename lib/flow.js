...
const profile = function profile(cb) {
  ...
  if (process.argv.slice(2).indexOf('-I') >= 0) {
    ...
    if (process.argv.slice(2).indexOf('-M') >= 0) {
      let maxConcurrent = process.argv.slice(2)[process.argv.slice(2).indexOf('-M') + 1];
      this.config.set(`profiles:base:maxConcurrent`, maxConcurrent || 2);
    }

    let nextGenProfiles = nextGenFilter.getNextGenTestCases(this.config.get('profiles:base'), this.config.get('data'), this.program.baseDirectory);
