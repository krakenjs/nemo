/**
 * I'd basically add my reporter hooks into this file down with all of the other runner.on(*) lines. My test object
 * would be shaped very similarly to what we already have here.
 */
...
module.exports = function instance(input, done, progress) {
  ...
  var exitInstance = function (failures, summary) {
    if (summary) {
      log('summary');
      progress({event: 'instance:end', payload: {
        tags: input.profile.tags,
  ...
  var defineSuite = function (config) {
    ...
    runner.on('pass', function (test) {
      log('pass');
      try {
        progress({event: 'pass', payload: prepareTestResult(test)});
      } catch (err) {
        error(err);
      }
    ...
    runner.on('fail', function (test, err) {
      error('fail', err);
      progress({event: 'fail', payload: prepareTestResult(test, err)});
      result.total = result.total + 1;
