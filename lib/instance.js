...
module.exports = function instance(instanceConfig, instanceComplete, instanceProgress) {
  ...
  emitter.on('beforetest', (test) => {
    instanceProgress({event: 'beforetest', payload: {tags: instanceConfig.profile.tags, test}});
  });
  emitter.on('beforesuite', () => {
    instanceProgress({event: 'beforesuite', payload: {tags: instanceConfig.profile.tags}});
  });
  emitter.on('suite', () => {
