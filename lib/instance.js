'use strict';
module.exports = function instance(input, doneThread, progress) {
  ...
  var defineSuite = function (config) {
    ...
    runner.on('pass', function (test) {
      ...
      try {
        progress({event: 'pass', payload: {tags: input.profile.tags, test: testCircularSafe}});
      } catch (err) {
        error(err);
      }
    ...
    runner.on('fail', function (test, err) {
      error('fail', err);
      let testCircularSafe = prepareTestResult(test);
      progress({event: 'fail', payload: {tags: input.profile.tags, test: testCircularSafe}});
      testResults.push(testCircularSafe);
    });
    ...
    runner.on('suite', function (Suite) {
      ...
      if (!(nemo && nemo.driver)) {
        Suite.beforeAll(function checkNemo(_doneMochaBeforeAll) {
